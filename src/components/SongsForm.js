...
class SongsForm extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if(nextProps.newSong.song && !nextProps.newSong.error) {
      this.context.router.push('/');
    }
  }

  renderError(newSong) {
    if(newSong && newSong.error && newSong.error.message) {
      return (
        <div className="alert alert-danger">
          {newSong ? newSong.error.message : ''}
        </div>
      );
    } else {
      return <span></span>
  ...
  render() {
    const {asyncValidating, fields: { title, categories, content }, handleSubmit, submitting, newSong } = this.props;

    return (
      <div className="container">
      {this.renderError(newSong)}
      <form onSubmit={handleSubmit(this.props.createSong.bind(this))}>
        ...
        <div className="alert alert-warning">
          NOTE: The app now has JWT Authentication turned on. <b>Please *Sign up* using some dummy data to create new songs</b>
        </div> 
      <br/>
      <br/>
      <br/>
